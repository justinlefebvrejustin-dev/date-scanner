...
# ==========================================
# 1. SETUP & STYLE (Originele mobiele focus)
# ==========================================
...
if img_file:
    ...
    with st.spinner('Searching for date...'):
        try:
            # FIX: Gebruik het model zonder 'models/' prefix als de library up-to-date is
            # Mocht dit nog falen, verander naar 'models/gemini-1.5-flash-latest'
            gemini = genai.GenerativeModel('gemini-1.5-flash')
            
            # Prompt verbeterd voor scherpere OCR resultaten
            prompt = """Scan the text on this image. 
Find the expiration date (EXP, THT, or best before). 
...
If no date is visible, reply with DATE: NULL"""
            
            res = gemini.generate_content([prompt, image_pil])
            response = res.text.strip()
            
            # Parsing (ongewijzigd qua logica, maar robuuster voor extra spaties)
            lines = response.split('\n')
            for line in lines:
                clean_line = line.replace('*', '').strip() # Verwijder markdown sterretjes
                if 'PRODUCT:' in clean_line.upper():
                    product_name_from_ai = clean_line.split(':', 1)[1].strip()
                if 'DATE:' in clean_line.upper():
                    date_text = clean_line.split(':', 1)[1].strip()
            
            # Check of datum echt gevonden is en cijfers bevat
            if date_text and 'NULL' not in date_text.upper() and any(c.isdigit() for c in date_text):
                date_found = True
                
        except Exception as e:
            # Toon error alleen als Gemini echt niet reageert
